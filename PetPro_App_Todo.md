...
- [ ] **Mass text (day-of cancellation)** — emergency use case: something happened, Nicole can't come in today, has to cancel every appointment on the day. Right now she'd have to text each client one by one. Build a "Mass Text" button on Calendar that: pulls every client with an appointment on the selected day (or date range), shows the list, lets her type one message, sends to all via Twilio in one shot. Real use: "husband's dad had heart surgery, had to cancel 10 dogs — mass text saved me."
