// =============================================================================
// PetPro Stripe Webhook Handler v1 — REPO BACKUP of the DEPLOYED function
// =============================================================================
// ⚠️ This function was built + deployed directly in the Supabase dashboard
// editor (April 22, 2026, Task #91) and lived ONLY there until June 11, 2026,
// when Nicole pasted it back so the repo finally has a copy. The DEPLOYED
// version in the Supabase dashboard is the source of truth — if you change
// this file, you must paste it into the dashboard editor to take effect.
//
// THIS is the long-lost code that:
//   • Grants AI token packs after Payment Link purchases (PACK_LINK_TO_TOKENS
//     + handleTokenPackCheckout) — verified working via the May 3 $1 test
//   • Syncs subscription tier + token + SMS allocations on subscribe/change
//   • Sends the welcome email
//   • Handles groomer→groomer referral rewards (30% off both, handoff #86)
//   • Zeroes monthly allocations on cancel (keeps paid top-ups + founder SMS)
//
// NOTE (Jun 11, 2026): TIER_TO_MONTHLY_SMS below still shows the OLD numbers
// (basic 0 / pro 1000 / pro_plus 1500 / growing 3000). Harmless — actual SMS
// allocation comes from the sync_sms_allocation_for_tier SQL RPC, which was
// updated to the new tiers (500/2000/3000/6000) in "SMS Quota Tier Update
// v2.sql". The map here is only used as an unknown-tier guard + log text.
//
...
//
// Deploy: Supabase Dashboard -> Edge Functions -> "stripe-webhook" -> editor.
//
...
// (STALE NUMBERS — see header note. Real allocation comes from the
// sync_sms_allocation_for_tier RPC; this map is only an unknown-tier guard.)
